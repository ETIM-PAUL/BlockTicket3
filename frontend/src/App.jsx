...
const App = () => {
  ...
  const provider = wallet?.accounts ? new ethers.providers.Web3Provider(
  ...
  const sendTransaction = async (data) => {
    ...
    try {
      ...
      if (rollups) {
        let str = `{"method":"ether_deposit","amount":${ethers.utils.parseEther(data.amount)}}`
  ...
  const etherTrf = async (data) => {
    ...
    try {
      ...
      if (rollups) {
        let str = `{"method":"ether_transfer","to":${data.to},"amount":${ethers.utils.parseEther(data.amount)}}`
        let payload = ethers.utils.toUtf8Bytes(str);
  ...
  const etherWit = async (data) => {
    ...
    try {
      ...
      if (rollups) {
        let str = `{"method":"ether_deposit","dapp":${data.dapp},"amount":${ethers.utils.parseEther(data.amount)}}`
        let payload = ethers.utils.toUtf8Bytes(str);
  ...
  const erc20Transfer = async (data) => {
    ...
    try {
      ...
      if (rollups) {
        let str = `{"method":"erc_transfer","from":${wallet.accounts[0].address},"to":${data?.dapp},"erc20":${data?.erc20},"amount":${ethers.utils.parseEther(amount)}}`
        let payload = ethers.utils.toUtf8Bytes(str);
  ...
  const erc20Withdraw = async (data) => {
    ...
    try {
      ...
      if (rollups) {
        let str = `{"method":"erc_withdraw","from":${data.dapp},"erc20":${data?.erc20},"amount":${ethers.utils.parseEther(amount)}}`
        let payload = ethers.utils.toUtf8Bytes(str);
  ...
  return (
    <div className="App bg-white h-screen">
      <TopNav />
      <span className='text-black font-bold text-lg md:text-2xl text-center block w-full mt-3'>Please connect your wallet to proceed</span>

      <GraphQLProvider>
        <span className='text-white text-lg md:text-2xl text-center block w-full font-bold'>Cartesi Vault</span>
        <select onChange={(e) => setSelected(e.target.value)} className="select select-bordered mx-auto grid mt-5 w-[300px]">
        ...
        {selected === "ether_dep" &&
          <form onSubmit={handleSubmit(sendTransaction)} className='border-gray-200 bg-gray-900 w-full md:max-w-2xl mx-auto mt-10 h-fit py-10'>
            <div>
              ...
              <div className='max-w-sm mx-auto mt-12'>
                {!wallet?.accounts ?
                  <button type="button" onClick={() => connect()} className={`bg-blue-500 w-full h-12 text-white font-bold text-xl rounded-md`}>Connect Wallet</button>
                  :
        ...
        {selected === "ether_trf" &&
          <form onSubmit={handleSubmit(etherTrf)} className='border-gray-200 bg-gray-900 w-full md:max-w-2xl mx-auto mt-10 h-fit py-10'>
            <div>
              <div className='px-4 pb-4 mt-10'>
              ...
              <div className='pt-4 max-w-sm mx-auto'>
                <div className='inline-grid w-full'>
                  <label className='text-white font-medium text-lg pb-1'>Receiver Address</label>
                  <input {...register("to")} className='h-12 w-full py-1 px-3 border rounded-md focus:outline-none' placeholder='Enter erc20 address' />
                </div>
                <div className='inline-grid w-full mt-4'>
                  <label className='text-white font-medium text-lg pb-1'>Ether Amount</label>

                  <div className='flex relative'>
                    <input {...register("amount")} type="number" className='h-12 w-full py-1 px-3 border rounded-md focus:outline-none appearance-none' placeholder='Enter Ether amount' />
                  </div>
                </div>
              </div>
              <div className='max-w-sm mx-auto mt-12'>
        ...
        {selected === "ether_wit" &&
          <form onSubmit={handleSubmit(etherWit)} className='border-gray-200 bg-gray-900 w-full md:max-w-2xl mx-auto mt-10 h-fit py-10'>
            <div>
              <div className='px-4 pb-4 mt-10'>
              ...
              <div className='pt-4 max-w-sm mx-auto'>
                <div className='inline-grid w-full'>
                  <label className='text-white font-medium text-lg pb-1'>Rollup Address</label>
                  <input {...register("dapp")} className='h-12 w-full py-1 px-3 border rounded-md focus:outline-none' placeholder='Enter rollup address' />
                </div>
                <div className='inline-grid w-full mt-4'>
                  <label className='text-white font-medium text-lg pb-1'>Ether Amount</label>
        ...
        {selected === "erc_trf" &&
          <form onSubmit={handleSubmit(erc20Transfer)} className='border-gray-200 bg-gray-900 w-full md:max-w-2xl mx-auto mt-20 h-fit py-10'>
            <div>
              <div className='px-4 pb-4 mt-10'>
              ...
              <div className='pt-4 max-w-sm mx-auto'>
                <div className='inline-grid w-full'>
                  <label className='text-white font-medium text-lg pb-1'>Receiver Address</label>
                  <input {...register("to")} className='h-12 w-full py-1 px-3 border rounded-md focus:outline-none' placeholder='Enter receiver address' />
                </div>
                <div className='inline-grid w-full mt-4'>
                  <label className='text-white font-medium text-lg pb-1'>ERC20 Amount</label>

                  <div className='flex relative'>
                    <input {...register("amount")} type="number" className='h-12 w-full py-1 px-3 border rounded-md focus:outline-none appearance-none' placeholder='Enter erc20 amount in the erc20 units' />
                  </div>
                </div>
                <div className='inline-grid w-full'>
                  <label className='text-white font-medium text-lg pb-1'>ERC20 Address</label>
                  <input {...register("erc20")} className='h-12 w-full py-1 px-3 border rounded-md focus:outline-none' placeholder='Enter erc20 address' />
                </div>
              </div>
              <div className='max-w-sm mx-auto mt-12'>
                {!wallet?.accounts ?
        ...
        {selected === "erc_with" &&
          <form onSubmit={handleSubmit(erc20Withdraw)} className='border-gray-200 bg-gray-900 w-full md:max-w-2xl mx-auto mt-20 h-fit py-10'>
            <div>
              <div className='px-4 pb-4 mt-10'>
                <span className='text-white text-lg md:text-2xl text-center block w-full font-bold'>ERC20 Withdraw</span>
              </div>
              <div className='pt-4 max-w-sm mx-auto'>
                {/* <div className='inline-grid w-full'>
                <label className='text-white font-medium text-lg pb-1'>Dapp Address</label>
